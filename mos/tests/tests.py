...
class EventFormTest(TestCase):
    ...
    def testFormCorrectData(self):
        ...
        assert f.is_valid() is True, 'Correct set of event data but form errors' 
        
    def testFormWrongData(self):
        """Creates a event form with invalid information"""
        f = EventForm(self.wrong_data)
        assert f.is_valid() is False, 'Name of the event is missing, but no error raised' 
        assert f.errors['name'] != "" , 'Name of the event is missing, but no error raised'
        assert f.errors['startDate'] != "" , 'Wrong date, but no error raised'
        assert f.errors['endDate'] != "" , 'Wrong time, but no error raised'
        assert f.errors['wikiPage'] != "" , 'Wikipage missing, but no error raised'
        print f.errors
...
class EventViewsTest(TestCase):
    ...
    def testAddNewEvent(self):
        ...
        response = self.c.post('/cal/new/',correct_data) # adds a new event via the view update_event
        self.assertContains(response, 'TestEvent1', count=None, status_code=200)
        self.assertContains(response, '06.06.2008 15:00', count=None, status_code=200)

        response = self.c.post('/cal/new/', self.minimal_data_set) # adds a new event via the view update_event
        self.assertContains(response, 'TestEvent1', count=None, status_code=200)
        self.assertContains(response, '06.06.2008 15:00', count=None, status_code=200)
